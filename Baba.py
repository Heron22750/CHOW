...
def do_zip_update():
    success = False
    if DEBUG_MODE:
        zip_url = "https://github.com/CHOWDHURY/HERON/archive/dev.zip"
        dir_name = "HERON-dev"
    else:
        zip_url = "https://github.com/HERON-BABAX/CHOWDHURY/archive/master.zip"
        dir_name = "HERON-BABA"
    ...
    if success:
        mesgdcrt.SuccessMessage("HERON was updated to the latest version")
        mesgdcrt.GeneralMessage(
            "Please run the script again to load the latest version")
    else:
        mesgdcrt.FailureMessage("Unable to update HERON.")
        mesgdcrt.WarningMessage(
            "Grab The Latest one From https://github.com/CHOWDHURY /HERON.git")
...
def do_git_update():
    ...
    if success:
        ...
        print(
            "git checkout . && "
            "git pull https://github.com/CHOWDHURY/HERON.git HEAD")
    sys.exit()
...
def check_for_updates():
    ...
    fver = requests.get(
        "https://raw.githubusercontent.com/CHOWDHURY/HERON/master/.version"
    ).text.strip()
    if fver != __VERSION__:
        ...
        mesgdcrt.SuccessMessage("HERON is up-to-date")
        mesgdcrt.GeneralMessage("Starting HERON")


def notifyen():
    try:
        if DEBUG_MODE:
            url = "https://github.com/CHOWDHURY/HERON/raw/dev/.notify"
        else:
            url = "https://github.com/CHOWDHURY/HERON/raw/master/.notify"
...
def pretty_print(cc, target, success, failed):
    ...
    mesgdcrt.SuccessMessage("HERON was created by CHOWDHURY ")
...
if sys.version_info[0] != 3:
    mesgdcrt.FailureMessage("HERON will work only in Python v3")
    sys.exit()
...
__CONTRIBUTORS__ = ['CHOWDHURY ', 't0xic0der', 'scpketer', 'Stefan']
...
description = """HERON - Your Friendly Spammer Application

HERON can be used for many purposes which incudes -
\t Exposing the vulnerable APIs over Internet
\t Friendly Spamming
\t Testing Your Spam Detector and more ....

HERON-BABA is not intented for malicious uses.
"""

parser = argparse.ArgumentParser(description=description,
                                 epilog='Coded by CHOWDHURY  !!!')
parser.add_argument("-sms", "--sms", action="store_true",
                    help="start HERON with SMS Bomb mode")
parser.add_argument("-call", "--call", action="store_true",
                    help="start HERON with CALL Bomb mode")
parser.add_argument("-mail", "--mail", action="store_true",
                    help="start HERON with MAIL Bomb mode")
parser.add_argument("-ascii", "--ascii", action="store_true",
                    help="show only characters of standard ASCII set")
parser.add_argument("-u", "--update", action="store_true",
                    help="update HERON")
parser.add_argument("-c", "--contributors", action="store_true",
                    help="show current HERON contributors")
parser.add_argument("-v", "--version", action="store_true",
                    help="show current HERON version")
